...
function setupEventListeners(product) {
  ...
  document.querySelector(".cart").addEventListener("click", function () {
    ...
    const existingItemIndex = cart.findIndex(
      (item) => item.id === cartItem.id && item.storage === cartItem.storage
    );
...
products: [
  {
    ...
    variants: [
      { storage: "256GB", price: 32000.0, stock: 15 },
      { storage: "512GB", price: 37000.0, stock: 10 },
      { storage: "1TB", price: 50000.0, stock: 20 },
    ],
  },
  {
    id: 2,
  ...
  function displayProductDetails() {
    ...
    const storageOptions = product.variants
      .map((v) => `<option value="${v.storage}">${v.storage}</option>`)
      .join("");
